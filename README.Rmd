...
The `set_gg()` function allows you to set a custom theme and palette for your `ggplot2` visualizations. It combines a theme with palette defaults using the theme palette entries introduced in ggplot2 4.0.0. Below is an example using the `theme_vm()` and the `vm` palette. For VM there is also the shortcut `set_vm()`.
